...
class DynamicPointLayer(MyModule):
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        sub_layer = nn.Conv2d(
            middle_channel, self.active_out_channel, self.kernel_size, stride=self.stride, bias=self.bias,
        )
        ...
        if self.bias:
            sub_layer.bias.data.copy_(
                self.point_linear.conv.conv.bias.data[:self.active_out_channel]
            )
